...
class Perceptron(object):
    ...
    def predict(self, token):
        ...
        prediction = res_dict[sort_res[0]][
            random.randint(0, len(res_dict[sort_res[0]]) - 1)
        ]
    ...
    def train(self, trainingData, numberOfIterations):
        ...
        for i in range(numberOfIterations + 1):
            ...
            for a in random.sample(range(len(trainingData)), len(trainingData)):
                for b in random.sample(
                    list(range(trainingData[a].length())), trainingData[a].length()
                ):
                    ...
                    for cl in pred[:-1]:
                        if cl != token.correctLabelIndex:
                            ...
                            self.weights.update(
                                cl, token.correctLabelIndex, token.features, -1.0
                            )
